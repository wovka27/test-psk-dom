  ...
  <q-dialog v-model="open">
    <q-card>
      <q-card-section class="row items-center q-pb-none">
        <div class="text-h6">
          {{ data.flats[id].type }} № {{ data.flats[id].number }}
          {{ data.flats[id].renovation ? 'с ремонтом' : '' }}
        </div>
        <q-space/>
      ...
      <q-card-section>
        <p>
          {{ data.flats[id].square }} м&#178; | {{ data.flats[id].floor }} этаж
        </p>
      </q-card-section>

      <q-card-section>
        <q-img
          src="https://kontakt-keramika.ru/800/600/https/pbs.twimg.com/media/EUS1BreX0Ag2uI1.jpg"
          width="500px"
          height="300px"
        />
      </q-card-section>
...
export default defineComponent({
  ...
  setup(props) {
    const {hovered, hoverTable, hoverCol, hoverRow, clearHover} =
      useHoverTable();
    ...
    const getCallback = (cb: (id: string) => void) => (id: string) => {
      if (
        filterData.length !== 0 &&
        props.data &&
        filterData.includes(props.data.flats[id])
      ) {
        cb(id);
